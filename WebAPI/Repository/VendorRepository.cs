...
namespace WebAPI.Repository
{
    public class VendorRepository : IVendorRepository
    {
        ...
        {
            var vendorIds = _context.VendorStocks.Where(v => v.ProductId == prodId && v.StockInHand > 0).Select(x => x.VendorId).ToList<int>();
            return await _context.Vendors.Where(v => vendorIds.Contains(v.VendorId)).ToListAsync();
        }
    }
}
